...
/*   Updated: 2020/12/02 16:13:17 by cserapon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	write_address(unsigned int address, unsigned int recursive)
{
	char	*hex;
	...
	if (address / 16 != 0)
	{
		write_address(address / 16, recursive + 1);
		if (recursive < 8 && hex[address % 16] == 'f')
			write(1, "0", 1);
		else
			write(1, &hex[address % 16], 1);
	}
...
void	full_address(void *addr, unsigned int count, unsigned int cursor)
{
	...
	address = (unsigned int)(addr + count);
